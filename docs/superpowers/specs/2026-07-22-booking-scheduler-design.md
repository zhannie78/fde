...
- Enforced via a Server Component layout (`/admin/(protected)/layout.tsx`, wrapping every admin page except `/admin/login`) plus a shared guard called at the top of every protected Route Handler — **not** Next.js Middleware. Netlify deploys Next.js Middleware as a Netlify Edge Function, and full `node:crypto` support there isn't something to rely on for the HMAC/timing-safe check; Route Handlers and Server Component rendering, by contrast, are documented to run in the Node.js runtime on Netlify. The check itself: read the cookie via `next/headers`'s `cookies()`, compare against the expected HMAC value with `crypto.timingSafeEqual`, redirect (layout) or return `401` (Route Handler) if missing or invalid.
